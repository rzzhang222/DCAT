...
from method.loss_doubleaugbetaregablationbeta0p01 import triple_loss
...
class WATrainer(object):
    ...
    def __init__(self, info, args, logger):
        ...
        #self.eval_attack = core.metrics.FGSM(self.get_model(), eps=8 / 255)
        #self.eval_attack = core.metrics.CW(self.get_model(),steps=10)
        #self.eval_attack = core.metrics.CW(self.get_model(),steps=20)
...
#                                             contrast_label=self.params.contrast_label)
            loss_dict, batch_metrics = triple_loss(use_str,criterion_mi=self.criterion_contrastive,
                                             ...
                                             step_size=self.params.attack_step,
                                             epsilon=self.params.attack_eps, perturb_steps=self.params.attack_iter,
                                             lamb=self.params.lamb, beta=0.01, attack=self.params.attack,
                                             contrast_label=self.params.contrast_label)                                 
