...
from method.loss_doubleaugnoreg import triple_loss
...
class WATrainer(object):
    ...
    def __init__(self, info, args, logger):
        ...
        self.eval_attack = core.metrics.PGD(self.get_model(), eps=8 / 255, alpha=1 / 255, steps=20)
        
        #self.eval_attack = core.metrics.AutoAttack(self.get_model(),norm='Linf', eps=8 / 255)
    ...
    def train(self, dataloader, use_str,epoch=0, verbose=False, logger=None):
...
#                                             contrast_label=self.params.contrast_label)
            loss_dict, batch_metrics = triple_loss(use_str,criterion_mi=self.criterion_contrastive,
                                             model=self.model, wamodel=self.wa_model,
                                             consistency_cost=self.params.consistency_cost * con_ramp,
                                             x_l=x_l, y=y_l, x_u=x_u, x_s=x_s,x_ss=x_ss,optimizer=self.optimizer,
                                             step_size=self.params.attack_step,
                                             epsilon=self.params.attack_eps, perturb_steps=self.params.attack_iter,
                                             lamb=self.params.lamb, beta=self.params.beta, attack=self.params.attack,
                                             contrast_label=self.params.contrast_label)                                 
    ...
    def tsne(self,dataloader, adversarial,model=None, verbose=True):
        model = model or self.get_model()
        model.eval()
        acc, total = 0.0, 0
        fea_all=None
        ...
        for data in tqdm(dataloader, desc='Eval : ', disable=not verbose):
            ...
            if adversarial:
               with ctx_noparamgrad_and_eval(self.wa_model):
                    x_adv = self.eval_attack(x, y)
               fea_nat, _ = model(x_adv, feats=True)   
            else:
               fea_nat, _ = model(x, feats=True)
            if fea_all==None:
                 fea_all=fea_nat.detach().cpu()
                 y_all=y
            else:
                 fea_all=torch.concat([fea_all,fea_nat.detach().cpu()],0) 
                 y_all=torch.concat([y_all,y],0)     
        X_tsne = tsne.fit_transform(fea_all)


        plt.figure(figsize=(16, 16))
        colors = ['red', 'green', 'blue','yellow','orange','grey','brown','skyblue','pink','purple']
        for i in range(len(colors)):
                plt.scatter(X_tsne[y_all == i, 0], X_tsne[y_all == i, 1], c=colors[i])
            #plt.legend()
        plt.savefig('tsne_dcat_clean_data_cifar10v2.png')
        #plt.savefig('tsne_wscat_adversarial_data_cifar10v2.png')
            #plt.show()
            #break
        model.train()
        return acc
...
#                    x_adv = self.eval_attack(x, y)
#            if adversarial:
#                    X_tsne=tsne.fit_transform(x_adv)        
#            else:
#                    X_tsne = tsne.fit_transform(x)
#            plt.figure(figsize=(8, 8))
#            colors = ['red', 'green', 'blue','yellow','orange','grey','brown','skyblue','pink','purple']
#            for i in range(len(colors)):
#                plt.scatter(X_tsne[y == i, 0], X_tsne[y == i, 1], c=colors[i])
#            #plt.legend()
#            plt.savefig('tsne_dcat_adversarial_datacifar10.png')
#            break
#        model.train()
#        return acc
...
#                plt.scatter(X_tsne[y == i, 0], X_tsne[y == i, 1], c=colors[i])
#            #plt.legend()
#            plt.savefig('tsnewscatcifar10.png')
#            break
#        model.train()
#        return acc
