...
from core.data import load_data,load_datadouble,load_datatriple
...
from core.trainer.watraindoubleaugbetareg import WATrainer
...
LOG_DIR = os.path.join(args.log_dir, args.desc+'augbetaregwithschedule6reg0.1scheduleablation60')
...
# Load data
seed(args.seed)
#train_dataset, test_dataset, eval_dataset, train_dataloader, test_dataloader, eval_dataloader = load_datadouble(
...
#    logger=logger,
#)
train_dataset, test_dataset, eval_dataset, train_dataloader, test_dataloader, eval_dataloader = load_datatriple(
    data_dir=DATA_DIR, batch_size=BATCH_SIZE, batch_size_test=BATCH_SIZE_VALIDATION,
    use_augmentation=args.augment,
    pseudo_label_model=pseudo_label_model,
    take_amount=args.take_amount, take_amount_seed=args.seed,
...
if resume_path is not None:
    if eval_dataloader and os.path.exists(WEIGHTS):
        ...
        print([old_score[0]*100,old_score[1]*100])
...
if NUM_ADV_EPOCHS >= start_epoch:
    ...
    for epoch in range(start_epoch, NUM_ADV_EPOCHS + 1):
        ...
        start = time.time()
        res = trainer.train(train_dataloader, use_str=use_str,epoch=epoch, logger=logger, verbose=True)
        for k in res:
            if 'acc' in k:
...
#        print([old_score[0]*100,old_score[1]*100])
#        logger.log(f'Best checkpoint resuming at epoch {best_epoch}. ')
#      start_epoch = trainer.load_model(WEIGHTS, weights_only=False) + 1
#      logger.log(f'(Strong) Resuming with strong aug at epoch {start_epoch - 1}')            
#            
#        #my add code
#      for epoch in range(start_epoch, NUM_ADV_EPOCHS + 1):
#        logger.log('======= Epoch {} ======='.format(epoch))
