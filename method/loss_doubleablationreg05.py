...
def triple_loss(criterion_mi, model, x_l, y, x_u, x_s,x_ss,optimizer,
        ...
        for _ in range(perturb_steps):
            ...
            with torch.enable_grad():
                ...
                if beta > 0:
                    ...
                    loss_con = criterion_mi(fea_nat, fea_adv, y=None, mask=mask)
...
#            labels_4_contrast = pseudo_labels.detach()
    ...
    loss3 = loss2+(loss_natural + lamb * loss_robust)*0.5
    
    #since i am doing ablation, comment the line after this
    loss=loss3+0.1*(criterion_kl(F.log_softmax(logits_adv2,dim=1),F.softmax(logits_adv,dim=1))+criterion_mi(fea_adv2,fea_adv,y=labels_4_contrast, mask=mask))
