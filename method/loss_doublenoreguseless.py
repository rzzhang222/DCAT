...
#            labels_4_contrast = pseudo_labels.detach()
    ...
    loss2 = loss1+(loss_natural + lamb * loss_robust)*1.0
    ...
    #loss=loss2+0.1*(criterion_kl(F.log_softmax(logits_adv2,dim=1),F.softmax(logits_adv,dim=1))+criterion_mi(fea_adv2,fea_adv,y=labels_4_contrast, mask=mask))
    loss=loss2  
#    fea_nat, logits_natural = model(x_natural, feats=True)
#    p_natural = F.softmax(logits_natural, dim=1).detach()
#
