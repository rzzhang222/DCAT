...
def triple_loss(criterion_mi, model, x_l, y, x_u, x_s,x_ss,optimizer,
    ...
    loss = loss2+(loss_natural + lamb * loss_robust)*0.5
    
    #since i am doing ablation, comment the line after this
    #loss=loss3+0.2*(criterion_kl(F.log_softmax(logits_adv2,dim=1),F.softmax(logits_adv,dim=1))+criterion_mi(fea_adv2,fea_adv,y=labels_4_contrast, mask=mask))
